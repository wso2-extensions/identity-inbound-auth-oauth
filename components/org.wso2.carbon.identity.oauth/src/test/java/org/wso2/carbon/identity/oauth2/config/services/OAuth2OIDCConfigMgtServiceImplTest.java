...
public class OAuth2OIDCConfigMgtServiceImplTest {
    ...
    private OAuth2OIDCConfigMgtServiceImpl oAuth2OIDCConfigMgtService;
    ...
    private MockedStatic<OAuth2OIDCConfigUtils> oAuth2OIDCConfigUtilsMock;
    ...
    public void setUp() throws Exception {
        ...
        // Explicitly open mocks to ensure @Mock annotations are processed
        ...
        oAuth2OIDCConfigUtilsMock = mockStatic(OAuth2OIDCConfigUtils.class);
        oAuth2OIDCConfigUtilsMock.when(() -> OAuth2OIDCConfigUtils.getIssuerLocation(anyString()))
        ...
        oAuth2OIDCConfigUtilsMock.when(() -> OAuth2OIDCConfigUtils.getDefaultIssuerUsageScopeConfig(anyString()))
                .thenAnswer(invocation -> {
                    String tenantDomain = invocation.getArgument(0);
        ...
        oAuth2OIDCConfigUtilsMock.when(() -> OAuth2OIDCConfigUtils.handleServerException(any(), any(), any()))
                .thenCallRealMethod();
        oAuth2OIDCConfigUtilsMock.when(() -> OAuth2OIDCConfigUtils.handleClientException(any(), any(), any()))
                .thenCallRealMethod();

        // Create the service instance after all mocks are set up
        oAuth2OIDCConfigMgtService = new OAuth2OIDCConfigMgtServiceImpl();
    }
    ...
    public void tearDown() {
        ...
        if (oAuth2OIDCConfigUtilsMock != null) {
            oAuth2OIDCConfigUtilsMock.close();
        }
    ...
    public void testGetOAuth2OIDCConfigs(Object resource, UsageScope expectedScope)
        ...
        OAuth2OIDCConfig result = oAuth2OIDCConfigMgtService.getOAuth2OIDCConfigs(TENANT_DOMAIN);

        assertNotNull(result, "OAuth2OIDCConfig should not be null");
        assertNotNull(result.getIssuerUsageScopeConfig(), "IssuerUsageScopeConfig should not be null");
        assertEquals(result.getIssuerUsageScopeConfig().getUsageScope(), expectedScope,
                "Usage scope should match expected value");
    }

    @Test(expectedExceptions = OAuth2OIDCConfigMgtServerException.class)
    public void testGetOAuth2OIDCConfigsException() throws Exception {
        ...
        when(configurationManager.getResource(
                OAuth2OIDCConfigConstants.ISSUER_USAGE_SCOPE_RESOURCE_TYPE_NAME,
                OAuth2OIDCConfigConstants.TENANT_ISSUER_USAGE_SCOPE_RESOURCE_NAME,
                false)).thenThrow(configException);

        oAuth2OIDCConfigMgtService.getOAuth2OIDCConfigs(TENANT_DOMAIN);
    }
    ...
    public void testUpdateOAuth2OIDCConfigsWithNull() throws Exception {
        ...
        try {
            oAuth2OIDCConfigMgtService.updateOAuth2OIDCConfigs(TENANT_DOMAIN, null);
            fail("Expected OAuth2OIDCConfigMgtClientException");
        } catch (OAuth2OIDCConfigMgtClientException e) {
            validateClientException(e, "60001",
                    String.format("Provided OAuth2 / OIDC configuration patch object is empty for %s.",
                            TENANT_DOMAIN));
        }
    }
    ...
    public void testUpdateOAuth2OIDCConfigsAddNewResource() throws Exception {
        ...
        OAuth2OIDCConfig oAuth2OIDCConfig = createOAuth2OIDCConfigWithScope(UsageScope.ALL_EXISTING_AND_FUTURE_ORGS);
        ...
        when(configurationManager.addResource(
                eq(OAuth2OIDCConfigConstants.ISSUER_USAGE_SCOPE_RESOURCE_TYPE_NAME),
                any(ResourceAdd.class))).thenReturn(new Resource());

        OAuth2OIDCConfig updatedOAuth2OIDCConfigs = oAuth2OIDCConfigMgtService.updateOAuth2OIDCConfigs(TENANT_DOMAIN,
                oAuth2OIDCConfig);

        assertNotNull(updatedOAuth2OIDCConfigs);
    ...
    public void testUpdateOAuth2OIDCConfigsReplaceExistingResource() throws Exception {
        ...
        OAuth2OIDCConfig oAuth2OIDCConfig = createOAuth2OIDCConfigWithScope(UsageScope.NONE);
        ...
        OAuth2OIDCConfig updateOAuth2OIDCConfigs = oAuth2OIDCConfigMgtService.updateOAuth2OIDCConfigs(TENANT_DOMAIN,
                oAuth2OIDCConfig);

        assertNotNull(updateOAuth2OIDCConfigs);
    ...
    public void testUpdateOAuth2OIDCConfigsToNoneScopeWithSubOrgApps() throws Exception {
        ...
        String subOrgId = "bd2de88d-89b8-4388-b9c3-fceecfaedd67";
        String subOrgTenantDomain = "sub-org.com";

        // Mock organization calls
        when(organizationManager.resolveOrganizationId(TENANT_DOMAIN)).thenReturn(ORG_ID);
        when(organizationManager.getChildOrganizationsIds(ORG_ID, true))
                .thenReturn(Collections.singletonList(subOrgId));
        when(organizationManager.resolveTenantDomain(subOrgId)).thenReturn(subOrgTenantDomain);
        ...
        when(applicationManagementService.getCountOfApplications(eq(subOrgTenantDomain), anyString(),
                any(), anyBoolean())).thenReturn(1);
        when(applicationManagementService.getApplicationBasicInfo(eq(subOrgTenantDomain), anyString(),
        ...
        when(applicationManagementService.getApplicationByResourceId(appBasicInfo.getApplicationResourceId(),
                subOrgTenantDomain)).thenReturn(serviceProvider);

        OAuthAppDO oAuthAppDO = new OAuthAppDO();
        oAuthAppDO.setIssuerOrg(ORG_ID);
        oAuth2UtilMock.when(() -> OAuth2Util.getAppInformationByClientId(CLIENT_ID, subOrgTenantDomain))
                .thenReturn(oAuthAppDO);

        try {
            oAuth2OIDCConfigMgtService.updateOAuth2OIDCConfigs(TENANT_DOMAIN, oAuth2OIDCConfig);
            fail("Expected OAuth2OIDCConfigMgtClientException");
        } catch (OAuth2OIDCConfigMgtClientException e) {
    ...
    public void testGetAllowedIssuers() throws Exception {
        ...
        lenient().when(privilegedCarbonContext.getTenantDomain()).thenReturn(subOrgTenantDomain);
        when(organizationManager.resolveOrganizationId(subOrgTenantDomain)).thenReturn(subOrgId);
        when(organizationManager.isPrimaryOrganization(subOrgId)).thenReturn(false);

        List<String> expectedIssuers = List.of(ISSUER_URL, subOrgIssuerUrl);
        when(orgResourceResolverService.getResourcesFromOrgHierarchy(eq(subOrgId), any(), any()))
    ...
    public void testGetAllowedIssuerDetails() throws Exception {
        ...
        IssuerDetails subOrgIssuerDetails = createIssuerDetails(subOrgIssuerUrl, subOrgId, subOrgTenantDomain);
        ...
        verifyIssuerDetails(result.get(1), subOrgIssuerUrl, subOrgId, subOrgTenantDomain);
    }
    ...
    private java.util.Optional<List<IssuerDetails>> invokeGetAllowedIssuerDetailsForOrg(String orgId) throws Exception {
        java.lang.reflect.Method method = OAuth2OIDCConfigMgtServiceImpl.class.getDeclaredMethod(
        ...
        try {
            ...
            if (cause instanceof OAuth2OIDCConfigMgtServerException) {
                throw (OAuth2OIDCConfigMgtServerException) cause;
    ...
    private Resource createResourceWithScope(UsageScope usageScope) {
        Attribute attribute = new Attribute(
                OAuth2OIDCConfigConstants.ISSUER_USAGE_SCOPE_USAGE_SCOPE_ATTRIBUTE,
                usageScope.getValue());
     ...
     */
    private OAuth2OIDCConfig createOAuth2OIDCConfigWithScope(UsageScope usageScope) {
        OAuth2OIDCConfig config = new OAuth2OIDCConfig();
        IssuerUsageScopeConfig issuerUsageScopeConfig = new IssuerUsageScopeConfig();
        issuerUsageScopeConfig.setUsageScope(usageScope);
        config.setIssuerUsageScopeConfig(issuerUsageScopeConfig);
        return config;
    }
    ...
    private ServiceProvider createServiceProviderWithOAuth2(String clientId) {
        ...
        requestConfig.setInboundAuthType(OAuth2OIDCConfigConstants.INBOUND_PROTOCOL_TYPE_OAUTH2);
     ...
     */
    private void ensureComponentHolderWiring() {
     ...
     */
    private void validateClientException(OAuth2OIDCConfigMgtClientException e, String expectedErrorCode,
    ...
    public void testGetAllowedIssuerDetailsForOrgWithConfigurationExceptionThrowsServerException() throws Exception {
        ...
        try {
            ...
            assertTrue(e.getCause() instanceof ConfigurationManagementException,
                    "Exception cause should be ConfigurationManagementException");
        }
    }
    ...
    public void testGetAllowedIssuerDetailsForOrgWithOrganizationExceptionThrowsServerException() throws Exception {
        ...
        // Only wire organizationManager since exception is thrown before configurationManager is used
        when(oAuth2ServiceComponentHolderInstance.getOrganizationManager()).thenReturn(organizationManager);
        ...
        try {
            ...
            // Expected exception
            assertNotNull(e.getMessage(), "Exception message should not be null");
            assertTrue(e.getMessage().contains("test-org-org-error-id") ||
                      e.getCause() instanceof OrganizationManagementException,
                      "Exception should be related to organization error");
        }
    }
}
