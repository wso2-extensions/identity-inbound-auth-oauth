...
public class OAuthEventPublishingUtilTest {
    ...
    public void setUp() throws UserIdNotFoundException {
        ...
        when(tokReqMsgCtx.getProperty(IdentityEventConstants.EventProperty.EXISTING_TOKEN_USED))
                .thenReturn(null);
    ...
    public void testPublishTokenIssueEvent() throws UserIdNotFoundException, IdentityEventException,
        ...
        try (MockedStatic<OAuth2Util> mockedOAuth2Util = Mockito.mockStatic(OAuth2Util.class);
            ...
            assertEquals("test-client-id", properties.get(IdentityEventConstants.EventProperty.CLIENT_ID));
            ...
            assertEquals("issuer-org-id",
                    properties.get(IdentityEventConstants.EventProperty.ISSUER_ORGANIZATION_ID));
            assertEquals("accessing-org-id",
                    properties.get(IdentityEventConstants.EventProperty.ACCESSING_ORGANIZATION_ID));
            assertEquals("APPLICATION_USER", properties.get(IdentityEventConstants.EventProperty.USER_TYPE));
            assertEquals("token-id-123", properties.get(IdentityEventConstants.EventProperty.TOKEN_ID));
