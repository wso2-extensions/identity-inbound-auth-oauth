...
public class AccessTokenIssuer {
    ...
    public OAuth2AccessTokenRespDTO issue(OAuth2AccessTokenReqDTO tokenReqDTO)
        ...
        try {
            ...
            if (StringUtils.isBlank(authorizedUser.getAuthenticatedSubjectIdentifier())) {
                if (!isOfTypeApplicationUser && useClientIdAsSubClaimForAppTokensEnabled) {
                    ...
                    String subjectIdentifier = OAuth2ServiceComponentHolder.getInstance().getClaimResolver()
                            .resolveSubjectClaim(getServiceProvider(tokReqMsgCtx.getOauth2AccessTokenReqDTO()),
                                    authorizedUser);
                    authorizedUser.setAuthenticatedSubjectIdentifier(subjectIdentifier);
                }
            }
