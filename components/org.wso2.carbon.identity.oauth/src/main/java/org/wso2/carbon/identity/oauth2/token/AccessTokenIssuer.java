...
public class AccessTokenIssuer {
    ...
    public OAuth2AccessTokenRespDTO issue(OAuth2AccessTokenReqDTO tokenReqDTO)
        ...
        if (!GrantType.AUTHORIZATION_CODE.toString().equals(grantType)) {
            ...
            // Clear the internal scopes. Internal scopes should only handle in
            ...
            if (isValidScope) {
                ...
                if (LoggerUtils.isDiagnosticLogsEnabled()) {
                    ...
                    LoggerUtils.triggerDiagnosticLogEvent(OAuthConstants.LogConstants.OAUTH_INBOUND_SERVICE, params,
                            OAuthConstants.LogConstants.FAILED, "Invalid scope provided in the request.",
                            "validate-scope", null);
                }
                tokenRespDTO = handleError(OAuthError.TokenResponse.INVALID_SCOPE, "Invalid Scope!",
                        tokenReqDTO);
                setResponseHeaders(tokReqMsgCtx, tokenRespDTO);
                triggerPostListeners(tokenReqDTO, tokenRespDTO, tokReqMsgCtx, isRefreshRequest);
                return tokenRespDTO;
