...
public class TokenBindingExpiryEventHandler extends AbstractEventHandler {
    ...
    public void handleEvent(Event event) throws IdentityEventException {
        ...
        try {
            if (request == null) {
                if (log.isDebugEnabled()) {
                    log.debug("HttpServletRequest object is null. Hence getting the session related information " +
                            "from event : " + event.getEventName() +
                            " and revoking the access tokens mapped to session");
                }
            ...
            if (sessionContext != null) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Session context for session context identifier: " + sessionContextIdentifier +
                            " is not found in the event : " + event.getEventName() + ".");
                }
            }
        }
    }
    ...
    private String getSessionIdentifier(Event event) {
        ...
        for (Map.Entry<String, Object> entry : paramMap.entrySet()) {
            if (StringUtils.equals(entry.getKey(), FrameworkConstants.AnalyticsAttributes.SESSION_ID)) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Found session context identifier: " + sessionContextIdentifier + " from the " +
    ...
    private void revokeTokensMappedToSession(String sessionId) throws IdentityOAuth2Exception {
        ...
        Set<String> tokenIds = OAuthTokenPersistenceFactory.getInstance().getAccessTokenDAO()
