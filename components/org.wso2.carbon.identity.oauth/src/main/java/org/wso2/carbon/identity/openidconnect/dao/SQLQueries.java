...
public class SQLQueries {
    ...
    public static final String STORE_IDN_OIDC_REQ_OBJECT_REFERENCE = "INSERT INTO IDN_OIDC_REQ_OBJECT_REFERENCE " +
            "(CONSUMER_KEY_ID, SESSION_DATA_KEY) SELECT ID,? FROM IDN_OAUTH_CONSUMER_APPS WHERE CONSUMER_KEY=?";
    ...
    public static final String STORE_IDN_OIDC_CLAIMS = "INSERT INTO IDN_OIDC_SCOPE_CLAIM_MAPPING " +
            "(SCOPE_ID, EXTERNAL_CLAIM_ID,TENANT_ID) SELECT ?,IDN_CLAIM.ID,? FROM IDN_CLAIM LEFT JOIN " +
            "IDN_CLAIM_DIALECT ON IDN_CLAIM_DIALECT.ID = IDN_CLAIM.DIALECT_ID WHERE CLAIM_URI=? AND IDN_CLAIM_DIALECT." +
            "DIALECT_URI='http://wso2.org/oidc/claim'";

    public static final String GET_ALL_IDN_OIDC_SCOPES = "SELECT TOP 1 ID FROM IDN_OIDC_SCOPE WHERE TENANT_ID=?";
    ...
    public static final String GET_IDN_OIDC_SCOPES_CLAIMS = "SELECT NAME,CLAIM_URI  FROM IDN_OIDC_SCOPE LEFT JOIN " +
            ...
            "IDN_CLAIM ON IDN_CLAIM.ID =IDN_OIDC_SCOPE_CLAIM_MAPPING.EXTERNAL_CLAIM_ID WHERE IDN_OIDC_SCOPE.TENANT_ID=?";
    ...
    public static final String GET_IDN_OIDC_CLAIMS = "SELECT CLAIM_URI  FROM IDN_OIDC_SCOPE LEFT JOIN " +
            "IDN_OIDC_SCOPE_CLAIM_MAPPING  ON IDN_OIDC_SCOPE_CLAIM_MAPPING.SCOPE_ID = IDN_OIDC_SCOPE.ID LEFT JOIN " +
            "IDN_CLAIM ON IDN_CLAIM.ID =IDN_OIDC_SCOPE_CLAIM_MAPPING.EXTERNAL_CLAIM_ID WHERE IDN_OIDC_SCOPE.TENANT_ID=?" +
            " AND IDN_OIDC_SCOPE.NAME=?";

    public static final String GET_IDN_OIDC_SCOPE_ID = "SELECT ID FROM IDN_OIDC_SCOPE WHERE NAME=? AND TENANT_ID=?";

    public static final String GET_OIDC_CLAIM_ID = "SELECT TOP 1 EXTERNAL_CLAIM_ID FROM IDN_OIDC_SCOPE_CLAIM_MAPPING LEFT JOIN" +
            " IDN_CLAIM ON IDN_OIDC_SCOPE_CLAIM_MAPPING.EXTERNAL_CLAIM_ID  = IDN_CLAIM.ID LEFT JOIN" +
            " IDN_CLAIM_DIALECT ON IDN_CLAIM_DIALECT.ID = IDN_CLAIM.DIALECT_ID WHERE  IDN_CLAIM_DIALECT.DIALECT_URI=" +
            "'http://wso2.org/oidc/claim' AND IDN_CLAIM .CLAIM_URI =?";

    public static final String GET_IDN_OIDC_SCOPE_ID_FOR_SCOPE = "SELECT TOP 1 IDN_OIDC_SCOPE.ID  FROM IDN_OIDC_SCOPE LEFT JOIN " +
            "IDN_OIDC_SCOPE_CLAIM_MAPPING  ON IDN_OIDC_SCOPE_CLAIM_MAPPING.SCOPE_ID = IDN_OIDC_SCOPE.ID LEFT JOIN " +
            "IDN_CLAIM ON IDN_CLAIM.ID =IDN_OIDC_SCOPE_CLAIM_MAPPING.EXTERNAL_CLAIM_ID WHERE IDN_OIDC_SCOPE.TENANT_ID=?" +
            " AND IDN_OIDC_SCOPE.NAME=? AND IDN_CLAIM.CLAIM_URI =?";
