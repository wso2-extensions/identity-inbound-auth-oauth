...
public class SQLQueries {
    ...
    public static final String STORE_IDN_OIDC_SCOPES = "INSERT INTO IDN_OIDC_SCOPE  " +
            "(NAME,TENANT_ID) VALUES (?, ?)";

    public static final String STORE_IDN_OIDC_CLAIMS = "INSERT INTO IDN_OIDC_SCOPE_CLAIM_MAPPING " +
            "(SCOPE_ID, EXTERNAL_CLAIM_ID,TENANT_ID) SELECT ?,ID,? FROM IDN_CLAIM WHERE CLAIM_URI=? AND DIALECT_ID=5";
    ...
    public static final String GET_IDN_OIDC_SCOPE_ID = "SELECT ID FROM IDN_OIDC_SCOPE WHERE NAME=?";

    public static final String GET_OIDC_CLAIM_ID = "SELECT EXTERNAL_CLAIM_ID FROM IDN_OIDC_SCOPE_CLAIM_MAPPING  LEFT JOIN" +
            "IDN_CLAIM ON IDN_OIDC_SCOPE_CLAIM_MAPPING.EXTERNAL_CLAIM_ID  = IDN_CLAIM.ID WHERE IDN_CLAIM.DIALECT_ID =5 " +
            "AND IDN_CLAIM .CLAIM_URI =?";

    public static final String DELETE_SCOPE_CLAIM_MAPPING = "DELETE FROM IDN_OIDC_SCOPE WHERE SCOPE=? AND TENANT_ID=?";
