...
public class SQLQueries {
    ...
    public static final String UPDATE_IDN_OAUTH2_SCOPE =
            "UPDATE IDN_OAUTH2_SCOPE SET DISPLAY_NAME=?, DESCRIPTION=?, TENANT_ID =? WHERE ID=?";
    ...
    public static final String GET_ALL_IDN_OIDC_SCOPES = "SELECT COUNT(ID) FROM IDN_OAUTH2_SCOPE " +
            "WHERE TENANT_ID=?";

    public static final String GET_IDN_OIDC_SCOPES_CLAIMS =
            "SELECT NAME, DISPLAY_NAME, DESCRIPTION, CLAIM_URI " +
                    "FROM IDN_OAUTH2_SCOPE " +
                    "LEFT JOIN IDN_OIDC_SCOPE_CLAIM_MAPPING " +
                    "ON IDN_OIDC_SCOPE_CLAIM_MAPPING.SCOPE_ID = IDN_OAUTH2_SCOPE.ID " +
                    "LEFT JOIN IDN_CLAIM " +
                    "ON IDN_CLAIM.ID = IDN_OIDC_SCOPE_CLAIM_MAPPING.EXTERNAL_CLAIM_ID " +
                    "LEFT JOIN IDN_CLAIM_DIALECT " +
                    "ON IDN_CLAIM_DIALECT.ID = IDN_CLAIM.DIALECT_ID " +
                    "WHERE IDN_OAUTH2_SCOPE.TENANT_ID = ? " +
                    "AND (IDN_CLAIM_DIALECT.TENANT_ID = ? " +
                    "AND IDN_CLAIM_DIALECT.DIALECT_URI = ?) " +
                    "OR (IDN_CLAIM_DIALECT.DIALECT_URI IS NULL AND IDN_CLAIM_DIALECT.TENANT_ID IS NULL)";

    public static final String GET_IDN_OIDC_SCOPES = "SELECT NAME FROM IDN_OAUTH2_SCOPE WHERE TENANT_ID=?";

    public static final String GET_IDN_OIDC_CLAIMS = "SELECT CLAIM_URI  FROM IDN_OAUTH2_SCOPE LEFT JOIN " +
            "IDN_OIDC_SCOPE_CLAIM_MAPPING  ON IDN_OIDC_SCOPE_CLAIM_MAPPING.SCOPE_ID = IDN_OAUTH2_SCOPE.ID " +
    ...
    public static final String GET_IDN_OIDC_SCOPE_ID = "SELECT ID FROM IDN_OAUTH2_SCOPE WHERE NAME=? " +
            "AND TENANT_ID=?";
    ...
    public static final String DELETE_SCOPE_AND_CLAIM_MAPPING =
            "DELETE FROM IDN_OAUTH2_SCOPE WHERE NAME=? AND TENANT_ID=?";
    ...
    public static final String DELETE_CLAIMS_FROM_SCOPE = "DELETE FROM IDN_OIDC_SCOPE_CLAIM_MAPPING WHERE " +
            ...
            "ON IDN_SCM.SCOPE_ID = IDN_OAUTH2_SCOPE.ID LEFT JOIN IDN_CLAIM " +
            ...
            "AND SCOPE_ID IN (SELECT IDN_OAUTH2_SCOPE.ID FROM IDN_OAUTH2_SCOPE WHERE NAME =?)";
    ...
    public static final String DELETE_CLAIM_MAPPING_OF_SCOPE = "DELETE FROM IDN_OIDC_SCOPE_CLAIM_MAPPING WHERE " +
            "SCOPE_ID= (SELECT ID FROM IDN_OAUTH2_SCOPE WHERE NAME=? AND TENANT_ID=?)";

    public static final String GET_IDN_OIDC_SCOPE_DETAILS = "SELECT NAME, DISPLAY_NAME, DESCRIPTION " +
            "FROM IDN_OAUTH2_SCOPE WHERE NAME=? AND TENANT_ID=?";

    public static final String RETRIEVE_BINDING_COUNT_OF_SCOPE =
            "SELECT COUNT(IDN_OAUTH2_SCOPE_BINDING.SCOPE_BINDING) FROM " +
                    "IDN_OAUTH2_SCOPE LEFT JOIN IDN_OAUTH2_SCOPE_BINDING ON " +
                    "IDN_OAUTH2_SCOPE.ID=IDN_OAUTH2_SCOPE_BINDING.SCOPE_ID WHERE IDN_OAUTH2_SCOPE.NAME = ? AND " +
                    "IDN_OAUTH2_SCOPE.TENANT_ID = ?";

    public static final String RETRIEVE_OIDC_CLAIMS_COUNT_OF_SCOPE = "SELECT COUNT(*) FROM " +
            "IDN_OIDC_SCOPE_CLAIM_MAPPING WHERE SCOPE_ID=? ";
