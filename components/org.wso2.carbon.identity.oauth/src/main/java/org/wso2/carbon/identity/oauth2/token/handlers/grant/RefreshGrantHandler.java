...
import org.wso2.carbon.identity.oauth2.processor.DefaultRefreshTokenGrantProcessor;
import org.wso2.carbon.identity.oauth2.processor.InMemoryRefreshTokenGrantProcessor;
import org.wso2.carbon.identity.oauth2.processor.RefreshTokenGrantProcessor;
...
public class RefreshGrantHandler extends AbstractAuthorizationGrantHandler {
    ...
    public static final String PREV_REFRESH_TOKEN = "previousRefreshToken";
    ...
    public RefreshGrantHandler() {
        if (OAuth2TokenUtil.isTokenPersistenceEnabled()) {
            refreshTokenGrantProcessor = new DefaultRefreshTokenGrantProcessor();
        } else {
            refreshTokenGrantProcessor = new InMemoryRefreshTokenGrantProcessor();
        }
    }
    ...
    public boolean validateGrant(OAuthTokenReqMessageContext tokReqMsgCtx)
        ...
        RefreshTokenValidationDataDO validationBean = refreshTokenGrantProcessor.validateRefreshToken(tokReqMsgCtx);
    ...
    public OAuth2AccessTokenRespDTO issue(OAuthTokenReqMessageContext tokReqMsgCtx)
        ...
        AccessTokenDO accessTokenBean = refreshTokenGrantProcessor.createAccessTokenBean(tokReqMsgCtx, tokenReq,
                validationBean, getTokenType());
        ...
        if (log.isDebugEnabled()) {
            log.debug("Generated an access token for the refresh token, " +
        ...
        refreshTokenGrantProcessor.persistNewToken(tokReqMsgCtx, accessTokenBean);
        setTokenDataToMessageContext(tokReqMsgCtx, accessTokenBean);
        refreshTokenGrantProcessor.addUserAttributesToCache(accessTokenBean, tokReqMsgCtx);
