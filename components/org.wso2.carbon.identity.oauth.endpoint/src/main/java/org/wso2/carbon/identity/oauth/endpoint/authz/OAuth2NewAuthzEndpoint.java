...
@Path("/authz")
public class OAuth2NewAuthzEndpoint {

    private static final Log log = LogFactory.getLog(OAuth2NewAuthzEndpoint.class);
    ...
    @Produces("text/html")
    public Response authorize(@Context HttpServletRequest request1, @Context HttpServletResponse response1)
            throws URISyntaxException, InvalidRequestException {

        OAuthMessage oAuthMessage = new OAuthMessage.OAuthMessageBuilder()
                .setRequest(request1)
        ...
        if (OAuthAuthorizeState.TO_COMMONAUTH.equals(oAuthMessage.getRequestType())) {
            return sendRequestToFramework(oAuthMessage);
        }

        SessionDataCacheEntry sessionDataCacheEntry = null;

        try {
            if (OAuthAuthorizeState.INITIAL_AUTHORIZATION_REQUEST.equals(oAuthMessage.getRequestType())) {// Authz request from client

                return handleIntialAuthorizationRequest(oAuthMessage);

            } else if (OAuthAuthorizeState.AUTHENTICATION_RESPONSE.equals(oAuthMessage.getRequestType())) {// Authentication response

                return handleAuthenticationResponse(oAuthMessage, sessionDataCacheEntry);

            } else if (OAuthAuthorizeState.USER_CONSENT_RESPONSE.equals(oAuthMessage.getRequestType())) { // Consent submission

                return handleResponseFromConsent(oAuthMessage, sessionDataCacheEntry);
            ...
            return handleOAuthSystemException(sessionDataCacheEntry, e);
    ...
    private Response handleResponseFromConsent(OAuthMessage oAuthMessage, SessionDataCacheEntry sessionDataCacheEntry) throws OAuthSystemException, URISyntaxException {
        String sessionDataKeyFromConsent = oAuthMessage.getRequest().getParameter(OAuthConstants.SESSION_DATA_KEY_CONSENT);
        SessionDataCacheEntry resultFromConsent = null;

        if (StringUtils.isNotEmpty(sessionDataKeyFromConsent)) {
            SessionDataCacheKey cacheKey = new SessionDataCacheKey(sessionDataKeyFromConsent);
        ...
        String consent = oAuthMessage.getRequest().getParameter("consent");
        if (consent != null) {
            ...
            String redirectURL =
                    handleUserConsent(oAuthMessage.getRequest(), consent, oauth2Params, sessionDataCacheEntry,
                            sessionState);
            ...
            if (isOIDCRequest) {
                ...
                redirectURL = manageOIDCSessionState(oAuthMessage.getRequest(), oAuthMessage.getResponse(),
                        ...
                        sessionDataCacheEntry.getLoggedInUser().getAuthenticatedSubjectIdentifier(), redirectURL);
            }
    ...
    private Response handleAuthenticationResponse(OAuthMessage oAuthMessage, SessionDataCacheEntry sessionDataCacheEntry) throws OAuthSystemException, URISyntaxException {
        String sessionDataKeyFromLogin = getSessionDataKey(oAuthMessage.getRequest());
        ...
        if (StringUtils.isNotEmpty(sessionDataKeyFromLogin)) {
            ...
            sessionDataCacheEntry = SessionDataCache.getInstance().getValueFromCache(cacheKey);
        }
        ...
        OAuth2Parameters oauth2Params = sessionDataCacheEntry.getoAuth2Parameters();
        AuthenticationResult authnResult = getAuthenticationResult(oAuthMessage.getRequest(), sessionDataKeyFromLogin);
        if (authnResult != null) {
            removeAuthenticationResult(oAuthMessage.getRequest(), sessionDataKeyFromLogin);
            ...
            if (authnResult.isAuthenticated()) {
                ...
                OIDCSessionState sessionState = new OIDCSessionState();
                redirectURL =
                        doUserAuthz(oAuthMessage.getRequest(), sessionDataKeyFromLogin, sessionDataCacheEntry,
                                sessionState);
    ...
    private Response handleIntialAuthorizationRequest(OAuthMessage oAuthMessage) throws OAuthSystemException, OAuthProblemException, URISyntaxException, InvalidRequestException {
        String clientId = oAuthMessage.getRequest().getParameter("client_id");
        String redirectURL = handleOAuthAuthorizationRequest(clientId, oAuthMessage.getRequest());
        ...
        if (attribute != null && attribute == AuthenticatorFlowStatus.SUCCESS_COMPLETED) {
            return sendRequestToFramework(oAuthMessage, type);
        } else {
            return Response.status(HttpServletResponse.SC_FOUND).location(new URI(redirectURL)).build();
        }
    }
    ...
    private void validateRepeatedParameters(OAuthMessage oAuthMessage) throws BadRequestException {
        if (!(oAuthMessage.getRequest() instanceof OAuthRequestWrapper)) {
            if (!EndpointUtil.validateParams(oAuthMessage.getRequest(), oAuthMessage.getResponse(), null)) {
     ...
     * @throws IOException
     */
    private Response sendRequestToFramework(OAuthMessage oAuthMessage) throws URISyntaxException, InvalidRequestException {
     ...
     * @throws IOException
     */
    private Response sendRequestToFramework(OAuthMessage oAuthMessage, String type) throws URISyntaxException,
